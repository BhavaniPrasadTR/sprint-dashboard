...
function Overview({ SPRINTS, CURRENT, TEAMS, GLOBAL, isDark }) {
  ...
  const prevSprint = SPRINTS[SPRINTS.length - 2] || lastSprint;
  const vTrend = prevSprint.vel ? (lastSprint.vel - prevSprint.vel) / prevSprint.vel : 0;
  const lastNumLabel = lastSprint.id.replace("S", "");
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:12 }}>
        {[
          { label:`${lastSprint.id} Velocity`,    value:f0(lastSprint.vel),      color:C.violet,  trend:vTrend,           trendGood:"up", sub:"vs prev sprint", tip:{ title:"Velocity", body:`Total story points delivered and accepted in Sprint ${lastNumLabel}. Higher means more work completed.`, formula:"Σ closed SP in sprint" } },
          { label:`${lastSprint.id} Completion`,  value:pct(lastSprint.cr),      color:lastSprint.cr>=1?C.emerald:lastSprint.cr>=0.8?C.amber:C.rose, trend:lastSprint.cr-prevSprint.cr, trendGood:"up", sub:"vs prev sprint", tip:{ title:"Sprint completion rate", body:"Percentage of committed story points actually delivered. 95% or above is excellent. Below 80% needs attention.", formula:"Velocity ÷ Committed SP" } },
          ...
          { label:"Predictability",   value:pct(lastSprint.cr),      color:lastSprint.cr>=0.95?"#059669":lastSprint.cr>=0.8?"#D97706":"#DC2626", trend:null, sub:`${lastSprint.id} · velocity÷committed`, tip:{ title:"Predictability index", body:"How consistently the team delivers what they commit to. 95% or above means forecasts are reliable for planning.", formula:"Velocity ÷ Committed SP" } },
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))", gap:12 }}>
        {[
          { label:`${lastSprint.id} Bugs`,          value:f0(lastSprint.bugs),  color:C.rose,            sub:`was ${f0(prevSprint.bugs)} in prev`, tip:{ title:"Sprint bugs", body:`Total bugs closed in Sprint ${lastNumLabel}. Compare with previous sprints to spot quality trends. Fewer bugs means higher quality.`, formula:"Count of Bug items closed" } },
          { label:`${lastSprint.id} Defect Density`,value:pct(lastSprint.dd),   color:QUAL(lastSprint.dd),      sub:"bugs÷closed items", tip:{ title:"Defect density", body:"Bugs as a percentage of all items closed. Below 25% is healthy (green), 25–40% is a watch zone (amber), above 40% needs action (red).", formula:"Bugs ÷ Total closed items" } },
          ...
          { label:`Bugs Closed ${lastSprint.id}`,   value:f0(lastSprint.bc),    color:C.emerald,         sub:`${pct(lastSprint.bc/lastSprint.bugs)} closure rate`, tip:{ title:"Bugs closed", body:"Bugs resolved and closed this sprint. A closure rate above 90% means quality debt is being kept under control.", formula:"Closed bugs ÷ Total sprint bugs" } },
...
function FlowPage({ SPRINTS, CURRENT, isDark }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))", gap:12 }}>
        {[
          { label:`Cycle P50 ${lastSprint.id}`,      value:`${f1(lastSprint.cy)}d`, color:lastSprint.cy<=14?C.emerald:C.amber, sub:`was ${f1(prevSprint.cy)}d prev`, tip:{ title:"Median cycle time", body:`Half of all ${lastSprint.id} items were completed within this many days of starting work. Target is 14 days or fewer.`, formula:"Median(days Active → Done)" } },
          { label:`Lead P50 ${lastSprint.id}`,       value:`${f1(lastSprint.ld)}d`, color:lastSprint.ld<=14?C.emerald:C.amber, sub:`was ${f1(prevSprint.ld)}d prev`, tip:{ title:"Median lead time", body:`Half of all ${lastSprint.id} items were completed within this many days of being created, including waiting time before work began.`, formula:"Median(days Created → Done)" } },
          { label:`Flow Efficiency ${lastSprint.id}`,value:`${f1(lastSprint.cy/lastSprint.ld*100)}%`, color:C.cyan, sub:"cycle÷lead time", tip:{ title:"Flow efficiency", body:"Percentage of total lead time that items were actively being worked on. 15–40% is typical for software teams. Very high values may indicate items were rushed.", formula:"Cycle time ÷ Lead time" } },
