...
const TEAM_DETAILS_DATA = [
  {team:"Dragons", user:"Agarwal, Pelak", role:"Dev"},
  {team:"Dragons", user:"Bhandari, Jayant (TR Technology)", role:"Dev"},
  {team:"Dragons", user:"Irabatti, Sunil", role:"Dev"},
  {team:"Dragons", user:"Kumar Chintala, Uday", role:"Dev"},
  {team:"Dragons", user:"Kumar, Ritesh", role:"QA"},
  {team:"Dragons", user:"Kurware, Deepak", role:"QA"},
  {team:"Dragons", user:"Lammers, John W. (TR Product)", role:"PO"},
  {team:"Dragons", user:"Maheshwari, Udit", role:"Dev"},
  {team:"Dragons", user:"Mishra, Brijendra", role:"Dev"},
  {team:"Dragons", user:"Nadapara, Harsh", role:"Dev"},
  {team:"Dragons", user:"Naidu Dulam, Manikanta", role:"Dev"},
  {team:"Dragons", user:"Raval, Nayan", role:"Dev"},
  {team:"Falcons", user:"Bring, Iker (TR Technology)", role:"Dev"},
  {team:"Falcons", user:"Cano, Sergio", role:"Dev"},
  {team:"Falcons", user:"Ganesan, Nirubaa (TR Technology)", role:"QA"},
  {team:"Falcons", user:"Gonzalez, Daniel (TR Product)", role:"PO"},
  {team:"Falcons", user:"Hernandez, Mauricio Alejandro (TR Technology)", role:"Dev"},
  {team:"Falcons", user:"Lara, Mariana (TR Technology)", role:"Dev"},
  {team:"Falcons", user:"Linares, Noe (TR Technology)", role:"Dev"},
  {team:"Falcons", user:"MuraliKrishnan, Athithyaraj", role:"Dev"},
  {team:"Falcons", user:"Nigam, Harshita", role:"Dev"},
  {team:"Falcons", user:"Ranka, Shruti", role:"QA"},
  {team:"Falcons", user:"Saxena, Aditi", role:"Dev"},
  {team:"Falcons", user:"Tankala, Harsha Vardhan", role:"QA"},
  {team:"Spartans", user:"Chura, Katsiaryna", role:"QA"},
  {team:"Spartans", user:"Ganduri, Keerthi (TR Technology)", role:"QA"},
  {team:"Spartans", user:"Gonzalez, Daniel (TR Product)", role:"PO"},
  {team:"Spartans", user:"Mk, Mitra (TR Technology)", role:"Dev"},
  {team:"Spartans", user:"Parate, Kamini", role:"QA"},
  {team:"Spartans", user:"Suggula, Madhuri (TR Technology)", role:"Dev"},
  {team:"Spartans", user:"Tasleem, Mohammad", role:"Dev"},
  {team:"Titans", user:"Akca, Reyhan", role:"Dev"},
  {team:"Titans", user:"Akula, Navya", role:"QA"},
  {team:"Titans", user:"Anshani, Hitesh", role:"Dev"},
  {team:"Titans", user:"Chowdhary, Harshitha", role:"Dev"},
  {team:"Titans", user:"Esakimuthu, Saran", role:"Dev"},
  {team:"Titans", user:"Kasam, Bhavana (TR Technology)", role:"Dev"},
  {team:"Titans", user:"Katroju, Siddartha", role:"QA"},
  {team:"Titans", user:"Madishetti, Mounika (TR Technology)", role:"Dev"},
  {team:"Titans", user:"Nagaraju, Ravikumarvarma", role:"Dev"},
  {team:"Titans", user:"Rajendran, Avishkar", role:"Dev"},
  {team:"Titans", user:"Shaik, Nagulameera", role:"QA"},
  {team:"Titans", user:"T C, Shabana Asmi (TR Technology)", role:"Dev"},
  {team:"Titans", user:"UmaKishore, Nakka", role:"Dev"},
  {team:"Titans", user:"Vemula, Rakesh Goud (TR Technology)", role:"QA"},
  {team:"Titans", user:"Wood, Matthew J. (TR Product)", role:"PO"}
];

const SPRINT_SCHEDULE = [
  {id:"S01",start:"2025-12-31",end:"2026-01-13"},{id:"S02",start:"2026-01-14",end:"2026-01-27"},
  {id:"S03",start:"2026-01-28",end:"2026-02-10"},{id:"S04",start:"2026-02-11",end:"2026-02-24"},
  {id:"S05",start:"2026-02-25",end:"2026-03-10"},{id:"S06",start:"2026-03-11",end:"2026-03-24"},
  {id:"S07",start:"2026-03-25",end:"2026-04-07"},{id:"S08",start:"2026-04-08",end:"2026-04-21"},
  {id:"S09",start:"2026-04-22",end:"2026-05-05"},{id:"S10",start:"2026-05-06",end:"2026-05-19"},
  {id:"S11",start:"2026-05-20",end:"2026-06-02"},{id:"S12",start:"2026-06-03",end:"2026-06-16"},
  {id:"S13",start:"2026-06-17",end:"2026-06-30"},{id:"S14",start:"2026-07-01",end:"2026-07-14"},
  {id:"S15",start:"2026-07-15",end:"2026-07-28"},{id:"S16",start:"2026-07-29",end:"2026-08-11"},
];
...
const CAP_STORE = "em_cap_v2";
function loadCap()  { try{return JSON.parse(localStorage.getItem(CAP_STORE)||"{}");}catch{return {};} }
function saveCap(d) { try{localStorage.setItem(CAP_STORE,JSON.stringify(d));}catch{} }

function CapacityPage({ TEAMS, isDark }) {
  const [sprint,  setSprint]  = useState("S14");
  const [team,    setTeam]    = useState("Falcons");
  const [cap,     setCap]     = useState(loadCap);
  const [editing, setEditing] = useState(null);
  const [editVal, setEditVal] = useState("");

  const spInfo  = SPRINT_SCHEDULE.find(sp => sp.id === sprint) || SPRINT_SCHEDULE[13];
  const totalWD = workingDays(spInfo.start, spInfo.end);
  const ROLE_CLR = {Dev:C.violet, QA:C.emerald, PO:C.amber};

  const capKey = (sp,u,f) => sp+"|"+u+"|"+f;
  const delKey = (sp,u)   => "deleted|"+sp+"|"+u;

  function getNum(sp,u,f) { return parseFloat(cap[capKey(sp,u,f)]||"0")||0; }

  function saveVal(sp,u,f,v) {
    const next = {...cap,[capKey(sp,u,f)]:String(v)};
    setCap(next); saveCap(next);
  }

  function commitEdit(u,f) {
    saveVal(sprint,u,f,parseFloat(editVal)||0);
    setEditing(null); setEditVal("");
  }

  function deleteRow(u) {
    const next = {...cap,[delKey(sprint,u)]:"1"};
    setCap(next); saveCap(next);
  }

  function restoreRow(u) {
    const next = {...cap}; delete next[delKey(sprint,u)];
    setCap(next); saveCap(next);
  }

  const allMembers     = TEAM_DETAILS_DATA.filter(m => m.team === team);
  const members        = allMembers.filter(m => !cap[delKey(sprint,m.user)]);
  const deletedMembers = allMembers.filter(m =>  cap[delKey(sprint,m.user)]);

  const [saving,     setSaving]    = useState(false);
  const [saveStatus, setSaveStatus]= useState(null); // null | "ok" | "err"

  // Collect ALL capacity rows across all sprints/teams from localStorage
  function collectAllRows() {
    const rows = [];
    const totalWDCache = {};
    SPRINT_SCHEDULE.forEach(sp => {
      const wd = totalWDCache[sp.id] || workingDays(sp.start, sp.end);
      totalWDCache[sp.id] = wd;
      TEAM_DETAILS_DATA.forEach(m => {
        if(cap[delKey(sp.id, m.user)]) return; // skip deleted
        const leave   = parseFloat(cap[capKey(sp.id,m.user,"leave")]  ||"0")||0;
        const holiday = parseFloat(cap[capKey(sp.id,m.user,"holiday")]||"0")||0;
        const spill   = parseFloat(cap[capKey(sp.id,m.user,"spill")]  ||"0")||0;
        // Only include rows that have at least one non-zero value or are in current/future sprint
        const hasDat = leave>0||holiday>0||spill>0;
        const isActive = sp.id >= sprint;
        if(!hasDat && !isActive) return;
        const availD = Math.max(0, wd - leave - holiday);
        const capSP  = availD;
        const netSP  = Math.max(0, capSP - spill);
        rows.push({
          team:sp.id>=sprint?m.team:m.team,
          user:m.user, role:m.role, sprint:sp.id,
          leave, holiday, spill,
          availDays:availD, capSP, netSP
        });
      });
    });
    return rows;
  ...
  async function saveToGitHub() {
    ...
    try {
      ...
      const owner = "BhavaniPrasadTR";
      const repo  = "sprint-dashboard";
      const resp  = await fetch(
        `https://api.github.com/repos/${owner}/${repo}/actions/workflows/save_capacity.yml/dispatches`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ref: "main",
            inputs: { capacity_json: JSON.stringify(rows) }
          })
        }
      );
      if(resp.status === 204 || resp.ok) {
        ...
        console.error("GitHub dispatch error:", resp.status, err);
        setSaveStatus("err");
  ...
  function memberCalc(u) {
    const leave   = getNum(sprint,u,"leave");
    const holiday = getNum(sprint,u,"holiday");
    const spill   = getNum(sprint,u,"spill");
    const availD  = Math.max(0, totalWD - leave - holiday);
    const capSP   = availD;                     // 1 SP = 1 day
    const netSP   = Math.max(0, capSP - spill);
    return {leave,holiday,spill,availD,capSP,netSP};
  }

  const totals = members.reduce((a,m)=>{
    const c=memberCalc(m.user);
    return {leave:a.leave+c.leave,holiday:a.holiday+c.holiday,
            spill:a.spill+c.spill,availD:a.availD+c.availD,
            capSP:a.capSP+c.capSP,netSP:a.netSP+c.netSP};
  },{leave:0,holiday:0,spill:0,availD:0,capSP:0,netSP:0});

  function EditCell({user, field, clr}) {
    const key = user+"|"+field;
    const val = getNum(sprint,user,field);
    if(editing===key) return (
      <div style={{display:"flex",gap:3,justifyContent:"flex-end",alignItems:"center"}}>
        <input autoFocus type="number" min={0} max={20} step={0.5} value={editVal}
          onChange={e=>setEditVal(e.target.value)}
          onKeyDown={e=>{if(e.key==="Enter")commitEdit(user,field);if(e.key==="Escape")setEditing(null);}}
          onBlur={()=>commitEdit(user,field)}
          style={{width:44,fontSize:11,textAlign:"center",padding:"2px 4px",
            background:"var(--bg)",color:"var(--text)",
            border:`1px solid ${C.border}`,borderRadius:4,fontFamily:"inherit"}} />
      </div>
    );
    return (
      <span onClick={()=>{setEditing(key);setEditVal(String(val||""));}}
        style={{textAlign:"right",fontSize:12,fontWeight:600,
          color:val>0?clr:C.dim,cursor:"pointer",display:"block",padding:"2px 0"}}
        title="Click to edit">
        {val>0?val:"—"}
      </span>
    );
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".05em"}}>Sprint</span>
          <select value={sprint} onChange={e=>setSprint(e.target.value)}
            style={{fontSize:12,fontWeight:600,background:"var(--surfaceM)",color:"var(--text)",
              border:`1px solid ${C.border}`,borderRadius:6,padding:"5px 12px",
              cursor:"pointer",fontFamily:"inherit"}}>
            {SPRINT_SCHEDULE.map(sp=>(
              <option key={sp.id} value={sp.id}>{sp.id} ({sp.start.slice(5)} to {sp.end.slice(5)})</option>
            ))}
          </select>
        </div>
        <div style={{display:"flex",gap:6}}>
          {TEAMS.map(t=>(
            <button key={t.name} onClick={()=>setTeam(t.name)}
              style={{fontSize:11,fontWeight:team===t.name?700:500,
                padding:"5px 14px",borderRadius:20,cursor:"pointer",
                border:`1.5px solid ${team===t.name?t.color:C.border}`,
                background:team===t.name?t.color+"18":"transparent",
                color:team===t.name?t.color:C.muted,transition:"all .15s"}}>
              {t.name}
            </button>
          ))}
        </div>
        <span style={{fontSize:11,color:C.muted,marginLeft:"auto"}}>
          <b>1 SP = 1 day</b> &nbsp;·&nbsp; Sprint has {totalWD} working days
        </span>
        {/* Save button */}
        <button onClick={saveToGitHub}
          disabled={saving}
          style={{fontSize:11,fontWeight:700,padding:"6px 16px",borderRadius:8,
            cursor:saving?"not-allowed":"pointer",
            background:saveStatus==="ok"?C.emerald:saveStatus==="err"?C.rose:C.violet,
            color:"#fff",border:"none",opacity:saving?0.7:1,transition:"all .2s",
            whiteSpace:"nowrap"}}>
          {saving?"Saving…":saveStatus==="ok"?"✓ Saved":saveStatus==="err"?"✗ Failed":"💾 Save Data"}
        </button>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12}}>
        {[
          {label:"Sprint",         val:sprint,                       clr:C.violet},
          {label:"Working Days",   val:totalWD+" days",              clr:C.sky},
          {label:"Members",        val:members.length+" active",     clr:C.text},
          {label:"Total Capacity", val:totals.capSP+" SP",           clr:C.emerald},
          {label:"Net Capacity",   val:totals.netSP+" SP",           clr:totals.netSP>0?C.emerald:C.rose},
        ].map(card=>(
          <div key={card.label} style={{background:"var(--surface)",borderRadius:12,
            padding:"14px 16px",border:`1px solid ${C.border}`}}>
            <div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",
              letterSpacing:".05em",marginBottom:6}}>{card.label}</div>
            <div style={{fontSize:18,fontWeight:800,color:card.clr}}>{card.val}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{background:"var(--surface)",borderRadius:12,
        ...
        <div style={{display:"grid",
          gridTemplateColumns:"1fr 60px 80px 80px 80px 90px 90px 72px",
          padding:"10px 16px",borderBottom:`2px solid ${C.border}`,
          fontSize:10,fontWeight:700,letterSpacing:".05em",
          textTransform:"uppercase",color:C.muted,background:"var(--surfaceM)"}}>
          <span>Member</span>
          <span style={{textAlign:"center"}}>Role</span>
          <span style={{textAlign:"right"}}>Leave Days</span>
          <span style={{textAlign:"right"}}>Holidays</span>
          <span style={{textAlign:"right"}}>Avail Days</span>
          <span style={{textAlign:"right",color:C.rose}}>Spillover SP</span>
          <span style={{textAlign:"right",color:C.emerald}}>Net Cap SP</span>
        ...
        <div style={{maxHeight:500,overflowY:"auto"}}>
          {members.length===0 ? (
            <div style={{padding:"40px 0",textAlign:"center",color:C.muted,fontSize:12}}>
              No active members for {team} in {sprint}. Restore below.
            </div>
          ) : members.map((m,idx)=>{
            const {leave,holiday,spill,availD,capSP,netSP} = memberCalc(m.user);
            const pct = capSP>0 ? Math.min(netSP/capSP,1) : 0;
            return (
              <div key={m.user} style={{display:"grid",
                gridTemplateColumns:"1fr 60px 80px 80px 80px 90px 90px 72px",
                ...
                <div>
                  ...
                  <div style={{height:3,background:C.border,borderRadius:2,marginTop:3,maxWidth:180}}>
                    <div style={{height:"100%",borderRadius:2,transition:"width .3s",
                      width:`${pct*100}%`,
                      background:pct>=0.8?C.emerald:pct>=0.5?C.amber:C.rose}} />
                  </div>
                </div>
                ...
                <div style={{textAlign:"center"}}>
                  <span style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:4,
                    background:(ROLE_CLR[m.role]||C.muted)+"22",
                    color:ROLE_CLR[m.role]||C.muted}}>
                    {m.role}
                  </span>
                </div>

                {/* Leave — click to edit */}
                <EditCell user={m.user} field="leave" clr={C.amber} />

                {/* Holidays — click to edit */}
                <EditCell user={m.user} field="holiday" clr={C.amber} />

                {/* Avail days — calculated */}
                <span style={{textAlign:"right",fontSize:12,fontWeight:700,
                  color:availD>0?C.text:C.rose}}>{availD}</span>

                {/* Spillover — click to edit */}
                <EditCell user={m.user} field="spill" clr={C.rose} />

                {/* Net SP */}
                <span style={{textAlign:"right",fontSize:13,fontWeight:800,
                  color:netSP>0?C.emerald:C.rose}}>{netSP}</span>

                {/* Delete */}
                <div style={{textAlign:"right"}}>
                  <button onClick={()=>deleteRow(m.user)}
        ...
        {members.length>0 && (
          <div style={{display:"grid",
            gridTemplateColumns:"1fr 60px 80px 80px 80px 90px 90px 72px",
            padding:"10px 16px",borderTop:`2px solid ${C.border}`,
            background:"var(--surfaceM)"}}>
            <span style={{fontSize:11,fontWeight:700,color:C.muted}}>
              TOTAL ({members.length} members)
            </span>
            <span/>
            <span style={{textAlign:"right",fontSize:12,fontWeight:700,
              color:totals.leave>0?C.amber:C.muted}}>{totals.leave||"—"}</span>
            <span style={{textAlign:"right",fontSize:12,fontWeight:700,
              color:totals.holiday>0?C.amber:C.muted}}>{totals.holiday||"—"}</span>
            <span style={{textAlign:"right",fontSize:12,fontWeight:700,color:C.text}}>{totals.availD}</span>
            <span style={{textAlign:"right",fontSize:12,fontWeight:700,
              color:totals.spill>0?C.rose:C.muted}}>{totals.spill||"—"}</span>
            <span style={{textAlign:"right",fontSize:13,fontWeight:800,color:C.emerald}}>{totals.netSP}</span>
            <span/>
          </div>
        )}
      </div>
      ...
      {deletedMembers.length>0 && (
        <div style={{background:"var(--surface)",borderRadius:10,
          ...
          <div style={{fontSize:11,fontWeight:700,color:C.muted,
            ...
            Removed members — click to restore
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {deletedMembers.map(m=>(
              <button key={m.user} onClick={()=>restoreRow(m.user)}
                style={{fontSize:11,padding:"4px 12px",borderRadius:6,cursor:"pointer",
                  background:"var(--surfaceM)",color:C.muted,
      ...
      {/* Legend */}
      <div style={{display:"flex",gap:20,flexWrap:"wrap",fontSize:11,color:C.muted}}>
        <span>💡 <b>Net Cap SP</b> = Available Days − Spillover SP</span>
        <span>📅 <b>Avail Days</b> = Working Days − Leave − Holidays</span>
        <span>✏️ Click any Leave / Holidays / Spillover value to edit inline</span>
        <span>💾 All changes auto-saved in browser localStorage</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ fontFamily:"'Inter','SF Pro Display',system-ui,sans-serif", background:"var(--bg)", minHeight:"100vh", color:"var(--text)", transition:"background .25s" }}>
      ...
      <div style={{ padding:"22px 24px 60px", maxWidth:1400, margin:"0 auto" }}>
        ...
        {tab==="capacity" && <CapacityPage TEAMS={TEAMS} isDark={isDark} />}
      </div>
    </div>
  );
}